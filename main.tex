...
control flow in instructions comprising multiple subtasks. Concretely, we examine problems in which programmatic
control-flow statements, e.g. \texttt{if} and \texttt{while} are evaluated
on the basis of observations of the environment and subsequently determine the subtask
that the agent must perform. Another challenging feature of the environments
that we study is that observations do not include explicit information about the current subtask to be performed.
...
Our work builds heavily on the work of \cite{oh2017zero} (henceforth, referred to as OLSK). That work considered
...
Our environment consists of a gridworld in which
...
\begin{enumerate}
  \item \textbf{Internal memory pointer:} Our architecture incorporates an
...
The architecture maps a $K$-length instruction
$\mathbf{I}$ to
...
This feature is similar to the internal memory pointer described in OLSK. In
that work, the
...
Movements of the internal memory pointer are a learned
...
\end{equation}
where $f_\theta$ is a convolutional neural network. We project $\mathbf{h}_t$ to $\mathbf{u}_t\in \mathbb{R}^\ell$, a weight vector
...
in order to assist in the execution of temporally-extended subtasks. We found it beneficial to incorporate gates into the algorithm that prevent changes
to certain internal parameters, especially when a subtask is not yet complete.

Our architecture incorporates two gates, one for the
    subtask parameter and one for the internal memory pointer.
...
c_g)\mathbf{g}_{t-1}$ and  $p_t = c_p\tilde{p}_t + (1 - c_p)p_{t-1}$. We train these
Bernoulli distributions with reinforcement learning (details in section
\ref{implementation}).


Moreover,  we divide the recurrent hidden state $\mathbf{h}_t$ into two
    parts, one updated every time-step and one updated only when the value of
    the gates is one. We condition the distribution from which the gate
