...
\newcommand{\algo}[3][
    ...
    \STATE $d_{t} \sim \Cat\left(\mathbf{W}_t\tilde{\mathbf{u}}_t\right)$ 
...
representation $\mathbf{H} \in \mathbb{R}^{K \times H\times 2}$ of the instruction
that incorporates local context information. The outputs of the GRU in both
directions are projected to a vector $\mathbf{B} \in (0, 1)^{K\times \ell\times
2 }$ using a sigmoidal
...
\begin{equation}
  ...
  \mathbf{B}_{kj}\right)
\end{equation}
where $\mathbf{P} \in \mathbb{R}^{K\times\ell\times 2 }$. Intuitively, $\mathbf{P}$ is the distribution produced by
