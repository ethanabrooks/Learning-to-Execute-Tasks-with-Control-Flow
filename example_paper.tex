...
pretrained lowerl-level policy maps the subtask parameter and the observation to
...
In \cite{oh2017zero}, the architecture generates a learned instruction memory
$\mathbf{M}$ that encodes each instruction $\mathbf{I}$ 
using ``Bag-of-words'' style
encodings applied to each subtask. Each subtask is
a list of words $\mathbf{i}_k = (w_1,\dots,w_{|\mathbf{i}_k|})$ and each
instruction is
a list of subtasks $\mathbf{I} = (\mathbf{i}_1,\dots,\mathbf{i}_{K})$. 
...
lines in the instruction, we maintain a single integer pointer $0 \le p_t < K$, where
$K$ is the length of the instruction,
...
Our architecture generates a graph representation of each instruction. That is,
from every line in the instruction, we produce
...
Our architecture employs rolling by rolling the instruction to the current memory index
...
the agent's capability to move the internal memory pointer through a instruction
