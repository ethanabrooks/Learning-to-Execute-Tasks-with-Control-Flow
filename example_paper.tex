...
thin, draw,
  ...
  \node[memory,label={[xshift=-19.0mm]right:Memory $\mathbf{M}$}]
    (M) at (-3, 2) {};
  \node[memory]                         (P) at (0, 22) {$\mathbf{P}$};
  \node at (-7, -1) {$p_t$};
  \node[bin]                            (Mp) at (-7, 2) {};
  \node[minimum size=1cm, draw,circle]  (x) at (6, 2) {$\mathbf{x}_t$};
  \node[draw,circle,minimum size=1cm]   (GRU) at (0, 10) {GRU};
  \fill [black!5]                      (-1,16) rectangle (-4.5,18);
  \fill [black!10]                      (0.5,16) rectangle (-3,18);
  \fill [black!20]                      (2,16) rectangle (-1.5,18);
  \fill [black!10]                      (3.5,16) rectangle (0,18);
  \fill [black!5]                      (5,16) rectangle (1.5,18);
  \draw [decorate,decoration={brace}]
  (-6,18.5) -- (6,18.5) node[midway,yshift=2mm]{\tiny softmax};
  \node[weights]                        (u) at (0, 17) {$\mathbf{u}_t$};
  \node                                 (p1) at (0, 30) {$d_{t}$};
  %\node                                 (cat) at (15, 30)
  %{$\Cat\left(\mathbf{P}\tilde{\mathbf{u}}_t\right)$};

  \draw [decorate,decoration={brace}]
  (-6,24.5) -- (6,24.5) node[midway,yshift=2mm]{};
  \node   (weighted-sum) at (0, 25.5) {\tiny weighted-sum};
...
\end{tikzpicture}
\caption{Diagram of the part of the architecture that maps the
...
\end{tikzpicture}
\caption{Architecture for mapping instruction $\mathbf{I}$ to encoded
instruction $\mathbf{M}$ and intuitive explanation of distribution from which
$d_t$ is sampled. Note that $\mathbf{P}$ contains $\ell$ such distributions and
