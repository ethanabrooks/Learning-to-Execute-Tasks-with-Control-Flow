...
\newcommand{\algo}[3][
    \STATE $V_t, \mathbf{u}_t, \mathbf{h}_t  \gets \GRU{\mathbf{x}_t, \mathbf{M}_{p_t}, \mathbf{h}_{t-1}}$
    %\COMMENT{
    ...
    \STATE $d_{t} \sim \Cat\left(\mathbf{W}_t\mathbf{u}_t\right)$ 
...
\begin{algorithm}[tb]
  ...
  \begin{algorithmic}[1]
    ...
    %\STATE number of lines per task $n \in \mathbb{N}$,
    %\STATE max graph arity $w \in \mathbb{N}$,
    %\STATE hidden-state size $h \in \mathbb{N}$,
    %\STATE embedded task matrix $\mathbf{M} \in \mathbb{R}^{n\times h}$,
...
\pgfdeclareimage[height=1cm]{goat}{figures/goat}
\pgfdeclareimage[height=1cm]{pig}{figures/pig}
\pgfdeclareimage[height=1cm]{cat}{figures/cat}
\pgfdeclareimage[height=1cm]{sheep}{figures/sheep}
\pgfdeclareimage[height=1cm]{agent}{figures/agent}

% The \icmltitle you define below is probably too long as a header.
% Therefore, a short form for the running title is supplied here:
\icmltitlerunning{Learning to Execute Tasks with Control Flow}


\begin{document}

\twocolumn[
\icmltitle{Hierarchical Reinforcement Learning with Complex Control Flow Instructions}
...
\begin{abstract}
  This research studies tasks comprising subtasks with nonlinear control-flow based on conditions in
  the environment. We present an algorithm capable not only of mastering these
  tasks, but of zero-shot generalization to longer tasks.
\end{abstract}
...
control flow in tasks comprising multiple subtasks. Whereas existing work
...
In keeping with \cite{oh2017zero}, we study tasks in which the observation does
...
\begin{enumerate}
  ...
  \item Identifying the next subtask to perform, which may occur several lines before (as in a
    \texttt{while} loop) or after the current line in the task.
\end{enumerate}
...
tasks. However, we demonstrate that these architectures do not generalize to
longer tasks. This paper presents a novel architecture capable of generalizing
to tasks multiple times the length of the longest training tasks.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[]
    ...
    \node at (3.5, 1) {\pgfbox[center,bottom]{\pgfuseimage{cat}}};
    \node at (2.5, 3) {\pgfbox[center,bottom]{\pgfuseimage{sheep}}};
    \node at (3.5, 3) {\pgfbox[center,bottom]{\pgfuseimage{pig}}};
    \node at (1.5, 2) {\pgfbox[center,bottom]{\pgfuseimage{agent}}};
\end{tikzpicture}
    %\caption{Level 1}
  \end{subfigure}%
  \hfill
...
\end{lstlisting}
  ...
  \end{subfigure}%
  \hfill
  \caption{Example task with environment}
\end{figure}
In order to study the problem of instructions with complex control-flow, we
study tasks based on a Domain Specific Language (DSL), similar to \cite{sun2020program}.
At the beginning of an episode our agent observes a numerical representation of
the task. Each time-step the agent also receives an observation of the
environment.

We assume a pretrained lower-level agent that will perform whatever subtask our
agent gives to it. Thus, our agent does not
choose the primitive actions to perform in the environment, but the higher-level
subtask that determines the behavior of the lower-level agent.

\subsection{Tasks}
Figure \ref{grammer} defines the grammar for tasks used in our domain.
Intuitively, tasks consist of lists of statements, which are either subtasks,
...
\caption{Grammar defining valid tasks}
\label{grammer}
\vskip -0.2in
\end{figure}
...
\section{Approach}
As stated earlier, conventional recurrent architectures are capable of
accomplishing the task described. However, what sets our architecture apart is
its capacity to generalize to larger tasks. Our architecture incorporates three
key insights:
...
\begin{enumerate}
  \item \textbf{Internal memory pointer:} Borrowing from \cite{oh2017zero}, our architecture incorporates an
    ...
    the larger task.
  \item \textbf{Task scanning:} When moving the internal memory pointer, the agent
    chooses the next line by ``scanning'' a learned representation of the  task and
  ...
  \item \textbf{Task rolling:} Our agent shifts or ``rolls'' the learned representation of the task
...
In \cite{oh2017zero}, the architecture generates a learned representation
$\mathbf{M}$ of the
task using ``Bag-of-words'' style
embeddings given numerical encodings of each subtask. The agent chooses actions
(subtask parameters) as a function of a weighted sum of these
embeddings. The agent learns a convolution operator 
...
lines in the task, we maintain a single integer pointer $0 \le p_t < K$, where
$K$ is the length of the task,
whose movements are drawn from a learned distribution. We found that this was beneficial, since a smooth shift might
cause the pointer to split along different paths of a conditional statement.

Moreover, because the next line in the task may not immediately follow or
...
\subsection{Task scanning}\label{task-scanning}
Our architecture generates a graph representation of each task. That is, from every line in the task, we produce
$\ell$ outgoing edges which each correspond to a learned distribution over other lines
in the task. To produce each distribution, we feed the embedded representation
of the task $\mathbf{M}$ through a bidirectional GRU to produce a representation of the task
that incorporates local context information. The outputs of the GRU in both
directions are projected to a vector $\mathbf{B} \in (0, 1)^{K\times \ell}$ using a sigmoidal
...
\begin{equation}
  ...
  \mathbf{B}_{kj}\right)
\end{equation}
where $\mathbf{P} \in \mathbb{R}^{K\times\ell}$. Intuitively, $\mathbf{P}$ is the distribution produced by
...
\begin{equation}
  \mathbf{h}_t = \GRU\left(f_\theta\left(\mathbf{x_t}\right), \mathbf{r}_{t-1}, \mathbf{g}_{t-1},
  \mathbf{h}_{t-1}\right)
\end{equation}
where $f_\theta$ is a convolutional neural network. Whereas \cite{oh2017zero}
projects $\mathbf{h}_{t}$ to the one-step convolution operator described in the
previous section, we project $\mathbf{h}_t$ to $\mathbf{u}_t\in \mathbb{R}^\ell$, a weight vector
used to choose among paths previously encoded from the task. Thus the final
distribution over next lines is $\Cat(\mathbf{P}_{p_t}\mathbf{u}_t)$, where
$p_t$ is the current memory index.

\subsection{Task rolling}
This feature takes advantage of the fact that the rules of control flow are
invariant to the position of the current line in the task. ``Rolling'' here
...
Our architecture employs rolling by rolling the task to the current memory index
$p_t$ before feeding it to the bidirectional GRU. Thus the forward GRU takes lines in the
order $(p_t, p_t + 1, \dots, K, 1, \dots, p_t - 1)$, running from the current
line, wrapping around the last line of the task, and returning to the line
before the current line. Thus the operations described in section \ref{task-scanning} are all performed
relative to the current active line in memory.
...
$\mathbf{H} \in \mathbb{R}^{2n \times w \times h}$ 
%\STATE $\mathbf{B} \gets \sigma\left(\Call{MLP}{\mathbf{H}}\right)$
\COMMENT{$\mathbf{B}\in \mathbb{R}^{a \times w}$}
\STATE $\mathbf{B}_{-1} \gets 1$  
\STATE initialize $\mathbf{W} \in \mathbb{R}^{2n \times w}$
\FOR{$i$ in $0, \dots, 2n$}
\STATE $\mathbf{W}_i \gets \mathbf{B}_i\prod_{j=1}^{i-1}\left(1 - \mathbf{B}_j\right)$
...
the agent's capability to move the internal memory pointer through a task
...
\begin{enumerate}
  \item \textbf{No scan:} This baseline removes the task scanning feature by
    ...
    Movements that go beyond the beggining or end of the task are clipped.
...
if-conditions and while-conditions pass, otherwise they fail.

\subsection{Complex Environment}
In this setting, the agent dictates subtask parameters to a pretrained
lower-level agent who executes them. Thus subtasks may take several turns to
